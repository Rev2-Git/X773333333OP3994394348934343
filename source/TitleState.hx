...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('newgrounds'));
		ngSpr.frames = Paths.getSparrowAtlas('newgrounds');
		ngSpr.animation.addByPrefix('idle', "ng logo", 25);
		add(ngSpr);
		ngSpr.visible = false;
		...
		slopSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('slopcrew_logo'));
		add(slopSpr);
		slopSpr.visible = false;
		slopSpr.setGraphicSize(Std.int(ngSpr.width * 0.8));
		slopSpr.updateHitbox();
		slopSpr.screenCenter(X);
		slopSpr.screenCenter(Y);
		slopSpr.antialiasing = ClientPrefs.globalAntialiasing;
